...
//##############################################################################
	...
	public function upgrade_options() {
		...
		if ( $this->upgrade_required() ) {
			...
			if ( $this->option_exists( 'cp_special_pages' ) ) {
				...
				// do we have the cp_sample_page option?

				if ( $this->option_exists( 'cp_sample_page' ) ) {



					// get it

					$page_id = $this->option_get( 'cp_sample_page' );
	...
	public function save_page_layout( $post ) {
		...
		if ( $post->ID == $this->option_get( 'cp_sample_page' ) ) {
	...
	public function create_special_pages() {
		...
		// create welcome/sample page, but don't add to special pages

		$welcome_sample = $this->_create_sample_page();
	...
	public function create_special_page( $page ) {
		...
		switch( $page ) {
			...
			case 'sample':
				...
				$new_id = $this->_create_sample_page();
	...
	public function delete_title_page()

	{

		

		 $page_id = $this->_create_title_page();
...

	{
		 ...
		 $page_idd = $this->_create_sample_page();

		

		return wp_delete_post($page_idd); 
    ...
    // Initialize mobile flag
    $is_mobile = false;
    
    // Initialize tablet flag
    $is_tablet = false;
    
    // Initialize touch flag
    ...
    if (isset($_SERVER["HTTP_USER_AGENT"])) {
        ...
        if (!class_exists('Mobile_Detect')) {
            // Include code from http://code.google.com/p/php-mobile-detect/
    ...
    // Update the values of the properties in the current instance
    // $this->is_mobile = $is_mobile;
    // $this->is_tablet = $is_tablet;
    // $this->is_mobile_touch = $is_mobile_touch;
}
...
//##############################################################################
	...
	function _create_title_page() {
		...
		if ( $page_exists !== false AND is_numeric( $page_exists ) ) {
			...
			$welcome = get_post( $page_exists );
				...
				if ( ! is_null( $welcome )) {
					...
					//if we comment this line so front end page not show
		...
		'<b>CommentarySandbox</b> is a WordPress plugin that creates a <a href="http://dcc.dickinson.edu/">Dickinson College Commentary</a>-style environment on a WordPress site. With the plugin you can juxtapose Note and Vocabulary tabs to Texts.



	The plug-in is free. Create, comment, and share. Click over to the “Sample Page” to see a simple commentary, with Text, Notes, and Vocabulary panes.



	You can find the plug-in on the <a href="https://wordpress.org/plugins/commentarysandbox/">WordPress.org plug-in directory</a> for download or to install directly into your WordPress site via your Plugins menu. If you wish to share your commentary, please consider submitting it to the <a href="http://iris.haverford.edu/library/">Haverford College Commentary Library</a>, which lists Open-Access commentaries on Latin and Greek texts.



	Sample commentaries made using the CommentarySandbox:

	<ul>

		<li><a href="http://iris.haverford.edu/echo/">Ovid’s Echo and Narcissus Episode</a></li>

		<li><a href="http://iris.haverford.edu/cologne-epode/">Archilochus’ Cologne Epode</a></li>

	</ul>

	The plug-in is a mod of the <a href="https://www.google.com/url?sa=t&amp;rct=j&amp;q=&amp;esrc=s&amp;source=web&amp;cd=1&amp;cad=rja&amp;uact=8&amp;ved=0ahUKEwj7jJrix6bRAhUBhyYKHU_jA4UQFggcMAA&amp;url=https%3A%2F%2Fwordpress.org%2Fplugins%2Fcommentpress-core%2F&amp;usg=AFQjCNFNFJ8ih2csyPMqrzxOpld3GB01BQ&amp;sig2=Gai-zYvouChjXJ-DfRNRNg">CommentPress Core plugin</a>, which allows crowdsourced commenting on a text.



	Additional documentation and support is available at: <a href="http://iris.haverford.edu/sandbox/">http://iris.haverford.edu/sandbox/</a>, including <a href="http://iris.haverford.edu/sandbox/commentarysandbox-instructions/">instructions</a>, how to efficiently <a href="http://iris.haverford.edu/sandbox/instructions-for-creating-efficient-and-accurate-vocabulary-lists-using-the-bridge/">create vocabulary lists</a>, and <a href="http://iris.haverford.edu/sandbox/help-with-creating-student-oriented-commentaries/">resources</a> for commentary creation.



	CommentarySandbox with developed 2017-2025 by Bret Mulligan (bmulliga@haverford.edu) and Mukesh Thakur.
...
', 'commentpress-core'
	...
	function _create_sample_page() {

		

	 	// get the option, if it exists

		$page_exists = $this->option_get( 'cp_sample_page' );
		...
		if ( $page_exists !== false AND is_numeric( $page_exists ) ) {
			...
			$welcome_sample = get_post( $page_exists );



			// check that the page exists

			if ( ! is_null( $welcome_sample ) ) {
		...
		$default_title = __( 'Sample Page', 'commentpress-core' );
		...
		'<strong>In the main text field:</strong>



		<em>The text goes here…</em>



		Using the WordPress editor, you can add text (with xml or html markup like formating, links, and more), add images, or anything else you would on a typical website.



		[1] HANNIBAL, Hamilcaris filius, Carthaginiensis. Si verum est, quod nemo dubitat, ut populus Romanus omnes gentes virtute superarit, non est infitiandum Hannibalem tanto praestitisse ceteros imperatores prudentia, quanto populus Romanus antecedat fortitudine cunctas nation.
		...
		<strong>(1) Hannibal: </strong>Most Carthaginian proper names are rendered in Latin as third declension nouns. In Punic, the language spoken by the Carthaginians, Hannibal meant something like "The Favorite of Baal." Baal Hammon was the chief god of the Carthaginians.
		...
		'<em>Add your Vocabulary here...</em>
		...
		(1) <strong>Hannibal Hannibalis m.</strong>: Hannibal
		...
		add_post_meta( $title_id, '_wp_editor_test_2', $vocub_content );

		

		add_post_meta( $title_id, '_wp_editor_test_1', $note_content );
		...
		if ( isset ( $_POST['_wp_editor_test_1'] ) ) {
				...
				update_post_meta( $title_id, '_wp_editor_test_1', $_POST['_wp_editor_test_1'] );
		...
		if ( isset ( $_POST['_wp_editor_test_2'] ) ) {
			...
			update_post_meta( $title_id, '_wp_editor_test_2', $_POST['_wp_editor_test_2'] );
		...
		$this->option_set( 'cp_sample_page', $title_id );
	...
	function _options_migrate() {
		...
		$sample_page =	isset( $old['cp_sample_page'] ) ? $old['cp_sample_page'] : null;
		...
		if ( ! is_null( $sample_page ) ) {
			...
			if ( get_post_meta( $sample_page, '_cp_post_type_override', true ) !== '' ) {
				...
				add_post_meta( $sample_page, '_cp_post_type_override', '0' );
			...
			$this->commentpress_options['cp_sample_page'] = $sample_page;
	...
	function _options_upgrade() {
		...
		$this->commentpress_options = array(
			...
			'cp_sample_page' => $this->option_wp_get( 'cp_sample_page'),
