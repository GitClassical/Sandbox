...
HELPTEXT;
	...
	public function list_pages( $exclude_pages = array() ) {
		...
		$sample_id = $this->db->option_get( 'cp_sample_page' );
		...
		if ( $welcome_id !== false AND $page_on_front == $welcome_id ) {
			...
			echo '<li class="hide_show page_item  page-item-' . $welcome_id . $is_active .'"><a href="' . get_permalink( $welcome_id ) . '">' . $title_page_title . '</a></li>';
...
//##############################################################################
	...
	function _get_admin_form() {
		...
		if ( $this->db->upgrade_required() ) {
			...
			<h1>' . __( 'Commentary Sandbox Settings', 'commentpress-core' ) . '</h1>
	...
	function _get_options() {
		...
		<p>' . __( 'When a supplied Commentary Sandbox theme (or a valid  child theme) is active, the following options modify its behaviour.', 'commentpress-core' ) . '</p>
	...
	function _get_reset() {
		...
		$label = __( 'Reset options to plugin defaults', 'Commentary Sandbox' );
	...
	function _get_button() {
		...
		$label = __( 'Hide and show pages', 'Commentary Sandbox' );
        ...
        global $wpdb;
            ...
            SELECT wp_posts.ID, wp_commentpress_new_field.page_id

            FROM wp_posts

            INNER JOIN wp_commentpress_new_field ON wp_posts.ID = wp_commentpress_new_field.page_id');
	...
	echo "<table>";
...

    	{
    	    ...
    	    $postpublish = array( 'ID' => $id_array, 'post_status' => 'publish' );

			 

			$postdraft = array( 'ID' => $id_array, 'post_status' => 'draft' );

			

    	     

    	  if (!isset($_POST['cp_radio_show'])) 
		    ...
		    <input id="cp_button_showw" class="searchTypee" name="cp_radio_show[]" value="0"

			type="checkbox" />Uncheck Post status publish and Check Post status Draft

			<p class="hide_number" style="display:none">'.wp_update_post($postpublish).'

			'.$page_exists = $this->db->option_get( 'cp_welcome_page' ).'

			'.$this->db->_store_wordpress_option( 'page_on_front',$page_exists ).'</p>
			...
			}elseif(isset($_POST['cp_radio_show']))
		    ...
		    <input id="cp_button_show" class="searchType" name="cp_radio_show[]" value="1"

			type="checkbox"/>Uncheck Post status publish and Check Post status Draft

			<p class="hide_number" style="display:none">'.wp_update_post($postdraft).'
	...
	function _get_editor() {
		...
		$editor_label = __( 'Comment form editor', 'Commentary Sandbox' );

		$rich_label = __( 'Rich-text Editor', 'Commentary Sandbox' );

		$plain_label = __( 'Plain-text Editor', 'Commentary Sandbox' );



		$behaviour_label = __( 'Default comment form behaviour', 'Commentary Sandbox' );

		$reading_label = __( 'Promote reading', 'Commentary Sandbox' );

		$commenting_label = __( 'Promote commenting', 'Commentary Sandbox' );
	...
	function _get_toc() {
		...
		$toc_label = __( 'Table of Contents contains', 'Commentary Sandbox' );

		$posts_label = __( 'Posts', 'Commentary Sandbox' );

		$pages_label = __( 'Pages', 'Commentary Sandbox' );



		$chapter_label = __( 'Chapters are', 'Commentary Sandbox' );

		$chapter_pages_label = __( 'Pages', 'Commentary Sandbox' );

		$chapter_headings_label = __( 'Headings', 'Commentary Sandbox' );



		$extended_label = __( 'Appearance of TOC for posts', 'Commentary Sandbox' );

		$extended_info_label = __( 'Extended information', 'Commentary Sandbox' );

		$extended_title_label = __( 'Just the title', 'Commentary Sandbox' );
	...
	function _get_sidebar() {
		...
		$label = __( 'Which sidebar do you want to be active by default? (can be overridden on individual pages)', 'Commentary Sandbox' );

		$contents_label = __( 'Contents', 'Commentary Sandbox' );

		$activity_label = __( 'Activity', 'Commentary Sandbox' );

		$comments_label = __( 'Comments', 'Commentary Sandbox' );
	...
	function _get_override() {
		...
		$label = __( 'Enable "live" comment refreshing (Please note: may cause heavy load on your server)', 'Commentary Sandbox' );
	...
	function _get_do_not_parse() {
			...
			$description = __( 'Note: when comments are closed on an entry and there are no comments on that entry, if this option is set to "Yes" then the content will not be parsed for paragraphs, lines or blocks. Comments will also not be parsed, meaning that the entry behaves the same as content which is not commentable. Default prior to 3.8.10 was "No" - all content was always parsed.', 'Commentary Sandbox' );
		...
		<tr valign="top">
			...
			<th scope="row"><label for="cp_do_not_parse">' . __( 'Disable Commentary Sandbox on entries with no comments. (can be overridden on individual entries)', 'Commentary Sandbox' ) . '</label></th>

			<td><select id="cp_do_not_parse" name="cp_do_not_parse">

					<option value="y" ' . (($this->db->option_get('cp_do_not_parse', 'n') == 'y') ? ' selected="selected"' : '') . '>' . __( 'Yes', 'Commentary Sandbox' ) . '</option>

					<option value="n" ' . (($this->db->option_get('cp_do_not_parse', 'n') == 'n') ? ' selected="selected"' : '') . '>' . __( 'No', 'Commentary Sandbox' ) . '</option>
	...
	function _get_page_nav_enabled() {
		...
		<tr valign="top">
			...
			<td><select id="cp_page_nav_enabled" name="cp_page_nav_enabled">
					...
					<option value="y" ' . (($this->db->option_get('cp_page_nav_enabled', 'y') == 'y') ? ' selected="selected"' : '') . '>' . __( 'Yes', 'Commentary Sandbox' ) . '</option>

					<option value="n" ' . (($this->db->option_get('cp_page_nav_enabled', 'y') == 'n') ? ' selected="selected"' : '') . '>' . __( 'No', 'Commentary Sandbox' ) . '</option>
	...
	public function _get_post_type_options() {
		...
		<tr valign="top">
			...
			<th scope="row"><label for="cp_post_types_enabled">' . __( 'Post Types on which Commentary Sandbox Core is enabled', 'commentpress-core' ) . '</label></th>
	...
	function _get_textblock_meta() {
		...
		//Create WSIWYG editing fieldsfor 
	...
	function _get_submit() {
		...
		$label = __( 'Save Changes', 'Commentary Sandbox' );
