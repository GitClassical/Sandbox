...
function commentpress_setup() {
	...
	if ( version_compare( $wp_version, '3.4', '>=' ) ) {
		...
		add_theme_support( 'custom-header', array(
			...
			'height' => apply_filters( 'cp_header_image_height', 140 ),
		...
		define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'cp_header_image_height', 140 ) );
...
function commentpress_enqueue_scripts_and_styles() {
	...
	wp_register_style(
		...
		plugin_dir_url( 'CommentarySandbox/themes/commentpress-modern/assets' ) . '/assets/css/screen' . $dev . '.css', // src
	...
	wp_enqueue_style( 'style_css', plugin_dir_url( 'CommentarySandbox/themes/commentpress-modern/assets' ) . '/assets/css/style.css',false,'1.1','all');

	

	//responsive add css

	wp_enqueue_style( 'responsive_css', plugin_dir_url( 'CommentarySandbox/themes/commentpress-modern/assets' ) . '/assets/css/responsive.css',false,'1.1','all');
	...
	wp_enqueue_style(
		...
		plugin_dir_url( 'CommentarySandbox/themes/commentpress-modern/assets' ) . '/assets/css/colours-01' . $dev . '.css',
	...
	);//stylecsswp_enqueue_style(		'style_style_css',		get_template_directory_uri() . '/assets/css/style' . $dev . '.css',		array( 'cp_webfont_style_css' ),		COMMENTPRESS_VERSION, // version		'all' // media	);
	...
	if ( is_object( $commentpress_core ) ) {
		...
		if ( $commentpress_core->db->is_special_page() ) {
			...
			wp_enqueue_script(
				...
				plugin_dir_url( 'CommentarySandbox/themes/commentpress-modern/assets' ) . '/assets/js/cp_js_all_comments.js',
...
function commentpress_enqueue_print_styles() {
	...
	wp_enqueue_style(
		...
		plugin_dir_url( 'CommentarySandbox/themes/commentpress-modern/assets' ) . '/assets/css/print' . $dev . '.css',
...
function commentpress_register_widget_areas() {
	...
	register_sidebar( array(
		...
		'name' => __( 'Commentary Sandbox Footer', 'commentpress-core' ),
...
/*function wpse_244394(){
    ...
    global $wpdb;
    	...
    	$result = $wpdb->get_results ( "SELECT * FROM wp_commentpress_new_field" );
